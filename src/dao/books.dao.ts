...
export class BooksDao {
    ...
    public getByCriteria(title?: string, author?: string, isbn?: string): BookModel[] {
        const books = this.databaseConnection.getData(`/books`)
        if (!title && !author && !isbn) {
            return books
        }
        return books.filter((book : BookModel )=> book.title === title || book.author === author || book.isbn === isbn)
